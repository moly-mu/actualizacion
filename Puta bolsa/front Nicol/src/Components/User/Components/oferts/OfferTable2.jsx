...
const OfferTable2 = ({ searchTerm }) => {
    ...
    useEffect(() => {
        const fetchOffers = async () => {
            ...
            const storedOffers = localStorage.getItem("offers2");
            ...
            if (storedOffers) {
                ...
                try {
                    ...
                    if (response.status === 200) {
                        const offersData = response.data.filter(offer => offer.job_board === "elempleo");

                        // Guardar las ofertas en localStorage
                        localStorage.setItem("offers2", JSON.stringify(offersData));
                        ...
                        currentOffers.map((offer, index) => (
                            <tr key={index} className="border-b">
                                <td className="px-4 py-2">
                                    <img src="/logoelempleopng.png" alt="Logo" className="w-12 h-12" />
                                </td>
                                <td className="px-4 py-2">
                                    <button
