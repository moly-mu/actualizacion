...
const OfferTable3 = ({ searchTerm }) => {
    ...
    useEffect(() => {
        const fetchOffers = async () => {
            ...
            const storedOffers = localStorage.getItem("offers3");
            ...
            if (storedOffers) {
                ...
                try {
                    ...
                    if (response.status === 200) {
                        const offersData = response.data.filter(offer => offer.job_board === "hoytrabajas");

                        // Guardar las ofertas en localStorage
                        localStorage.setItem("offers3", JSON.stringify(offersData));
                        ...
                        currentOffers.map((offer, index) => (
                            <tr key={index} className="border-b">
                                <td className="px-4 py-2">
                                    <img src="/logohoytrabajas.png" alt="Logo" className="w-12 h-12" />
                                </td>
                                <td className="px-4 py-2">
                                    <button
