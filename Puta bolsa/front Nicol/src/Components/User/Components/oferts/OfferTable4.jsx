...
const OfferTable4 = ({paginate, currentPage, filteredOffersLength, offersPerPage, handleApplyToOffer, handleViewDescription }) => {
    ...
    useEffect(() => {
        const fetchOffers = async () => {
            try {
                ...
                setOffers(response.data); // Asignar las ofertas al estado
            } catch (error) {
                console.error("Error al obtener las ofertas:", error.message);
                Swal.fire("Error", "Hubo un problema al cargar las ofertas.", "error");
            }
        };

        fetchOffers(); // Llamada a la función de obtención de datos
