...
const dataSchema = new mongoose.Schema({
    ...
    Keyword: [String],  // Array de palabras clave
...
async function insertarVacantesDesdeJSON(filePath) {
    ...
    try {
        for (const oferta of data) {
            // Verificar si la oferta ya existe en la base de datos con todos los campos relevantes
            const ofertaExistente = await Data.findOne({
                Title: oferta.Title,
                Description: oferta.Description,
                Location: oferta.Location,
                ClosingDate: oferta.ClosingDate,
