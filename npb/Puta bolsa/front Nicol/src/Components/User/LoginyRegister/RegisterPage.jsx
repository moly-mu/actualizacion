...
const RegisterPage = ({ onLoginClick, onRegister }) => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/auth/register/user', userData);
      ...
      if (response.status === 200) {
        ...
        Swal.fire({
          ...
          text: response.data.message || "Error al registrar el usuario.",
          confirmButtonText: "Aceptar",
          confirmButtonColor: "#3b82f6",
        });
      }
  ...
  return (
<div className="flex items-center justify-center min-h-screen bg-gradient-to-r from-blue-50 to-blue-100 py-10">
  <div className="flex flex-col md:flex-row items-center bg-white shadow-2xl rounded-3xl overflow-hidden w-full max-w-5xl h-auto md:h-[950px]">
    {/* Imagen lateral con altura ajustada */}
    <div className="hidden md:block md:w-2/3 lg:w-1/2 relative">
      <img
        src="/no.jpeg" // Ruta a la imagen dentro de la carpeta public
        alt="Imagen usuario"
        className="h-full w-full object-cover rounded-l-3xl transform transition-transform duration-300 ease-in-out hover:scale-105"
      />
      <div className="absolute inset-0 bg-black opacity-20 rounded-l-3xl"></div> {/* Fondo oscuro para mejorar el contraste */}
    </div>

    {/* Formulario de registro */}
    <div className="w-full md:w-1/2 lg:w-2/3 p-8 md:p-10 relative flex flex-col justify-center">
      {/* Botón para regresar */}
      <button
        onClick={() => navigate("/login")} // Navegar siempre a la página principal
        className="absolute top-12 left-4 p-2 text-gray-700 hover:text-gray-900 transition duration-300 ease-in-out rounded-full bg-white shadow-md"
      >
      ...
      <h2 className="text-4xl font-bold text-center text-gray-800 mb-6">
        Registrarse
          ...
          <div className="mt-6">
            <button className="w-full px-4 py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-300 mb-4">
              <FcGoogle size={24} className="mr-2 inline-block" />
